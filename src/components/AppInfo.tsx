      ...
      <div className="space-y-3 pb-8 text-[13px] leading-6 text-slate-600 dark:text-slate-300">
        <section id="help" data-testid="help-section" className={`${cardClass} scroll-mt-16`}>
          ...
          <ol className="list-decimal space-y-2 pl-5">
            <li>先在“数据治理与备份”设置操作人员、仪器档案和规则版本，再新建测流或水准任务。</li>
            <li>录入后查看实时校核。出现缺失、断链或超限时先处理提示，再完成并锁定成果。</li>
            <li>重要任务完成后导出Excel，并在治理中心生成完整备份；恢复操作会先校验文件完整性。</li>
            <li>水准测站GPS仅在点击定位建站时请求。拒绝权限后自动降级为无坐标建站，不影响读数录入。</li>
            <li>电子气泡仅作辅助校准。设备不支持、权限拒绝或无传感器数据时不会生成模拟读数。</li>
          </ol>
          <p className="mt-3 rounded-xl bg-blue-50 px-3 py-2 text-xs text-blue-700 dark:bg-blue-950/30 dark:text-blue-300">故障处理：先导出脱敏诊断和完整备份，再联系支持人员。不要在未备份时清除应用数据或卸载。</p>
        </section>
        ...
        <section id="privacy" data-testid="privacy-section" className={`${cardClass} scroll-mt-16`}>
          ...
          <ul className="mt-1 list-disc space-y-1 pl-5">
            <li>用户主动录入的测流、水准、测点、人员、仪器、规则、备注和成果数据。</li>
            ...
            <li>本地脱敏诊断仅含版本、平台、记录数量和错误类别，不含完整读数、备注、姓名或仪器序列号。</li>
          </ul>
          <h3 className="mt-3 font-bold text-slate-700 dark:text-slate-100">存储、共享与期限</h3>
          <p>业务数据默认保存在本机。只有用户主动导出Excel、备份或诊断时才进入系统分享流程。数据保留至用户删除、恢复覆盖、清除应用数据或卸载；应用不自动上传业务成果。</p>
        ...
        <section id="agreement" data-testid="agreement-section" className={`${cardClass} scroll-mt-16`}>
          ...
          <ul className="list-disc space-y-1 pl-5">
            ...
            <li>成果归档前应完成业务复核和备份。因误输入、错误参数、未备份或设备损坏导致的风险应按单位制度处置。</li>
            <li>不得利用应用处理无权访问的数据、破坏更新校验、绕过成果审计或传播恶意文件。</li>
            <li>版本升级可能调整数据结构；应用提供迁移和备份能力，重大升级前仍应由用户主动导出完整备份。</li>
          </ul>
        </section>
