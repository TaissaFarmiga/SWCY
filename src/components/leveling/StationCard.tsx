...
function GlassInput({
  ...
  return (
    <div className="flex items-center gap-1.5 w-full">
      <span className="text-[10px] text-slate-400 dark:text-slate-500 w-6 shrink-0">{label}</span>
      <div className="relative flex-1 min-w-[50px]">
        <input
          ...
          className={`w-full pl-1.5 pr-4 py-1 rounded-md shadow-inner text-center font-mono text-[12px] font-bold outline-none transition-colors ${
            isError
              ? 'bg-red-50/80 dark:bg-red-900/30 border border-red-400 text-red-600 dark:text-red-400'
              : 'bg-white/80 dark:bg-gray-900/50 border border-slate-200 dark:border-gray-700 text-slate-700 dark:text-slate-200 focus:border-hydro-blue'
          }`}
        />
        {unit && <span className="absolute right-1.5 top-1/2 -translate-y-1/2 text-[9px] text-slate-400 pointer-events-none font-sans">{unit}</span>}
      </div>
    </div>
  );
}
...
function ConfirmDeleteButton({ onDelete }: { onDelete: () => void }) {
  ...
  return confirming ? (
    <button aria-label="确认删除测站" onClick={() => { if (timerRef.current) clearTimeout(timerRef.current); setConfirming(false); onDelete(); }}
      className="min-h-11 min-w-11 p-1 rounded bg-red-500 text-white text-[10px] font-bold animate-pulse shrink-0">确认删除？</button>
  ) : (
    <button aria-label="删除测站" onClick={(e) => { e.stopPropagation(); setConfirming(true); timerRef.current = setTimeout(() => setConfirming(false), 3000); }}
      className="flex min-h-11 min-w-11 items-center justify-center rounded hover:bg-red-50 dark:hover:bg-red-900/30 text-slate-400 hover:text-red-500">
    ...
    >
      ...
      <div className="flex items-center justify-between px-2 py-1.5 border-b border-slate-100/50 dark:border-gray-700/50">
        <div className="flex items-center gap-2">
          ...
          <button
            type="button"
            onClick={() => setStationDirection(station.id, station.direction === 'forward' ? 'return' : 'forward')}
            className={`min-h-11 min-w-11 rounded-lg px-2 text-[10px] font-bold ${station.direction === 'return' ? 'bg-violet-100 text-violet-700 dark:bg-violet-900/40 dark:text-violet-300' : 'bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300'}`}
            title="切换往测/返测"
          >
      ...
      {(readings.intermediates?.length || 0) > 0 && (
        <div className="flex flex-col border-t border-violet-100/50 dark:border-violet-900/30 bg-violet-50/30 dark:bg-violet-900/10">
          {(readings.intermediates || []).map((inter, i) => {
            ...
            return (
              <div key={inter.id} className="relative flex flex-col gap-1.5 p-1.5 border-b border-violet-100/50 dark:border-violet-900/30 last:border-0">
                ...
                <div className="flex items-center gap-1.5">
                  <div className="flex-[4]"><GlassInput label="测点" value={inter.point} onChange={(v) => updateIntermediate(station.id, inter.id, 'point', v)} isError={isStaffErr} /></div>
                  <div className="flex-[3]"><GlassInput label="黑面" value={inter.black} onChange={(v) => updateIntermediate(station.id, inter.id, 'black', v)} isError={isStaffErr} /></div>
                  <div className="flex-[3]"><GlassInput label="红面" value={inter.red} onChange={(v) => updateIntermediate(station.id, inter.id, 'red', v)} isError={isStaffErr} /></div>
                  <button onClick={() => removeIntermediate(station.id, inter.id)} className="p-1.5 text-slate-300 hover:text-red-500 active:scale-95 shrink-0 transition-colors">
                    <Trash2 className="w-3.5 h-3.5" />
                  </button>
                </div>
              </div>
      ...
      <div onClick={() => setShowResults(!showResults)}
        className="w-full flex items-center justify-between cursor-pointer select-none py-1.5 px-2 bg-slate-50/80 dark:bg-gray-800/50 hover:bg-slate-100 dark:hover:bg-gray-800/80 transition-colors border-t border-slate-100/50 dark:border-gray-700/50">
        ...
        <div className="flex items-center gap-1.5 shrink-0" onClick={(e) => e.stopPropagation()}>
          <button
            ...
            className="flex min-h-11 min-w-11 items-center gap-0.5 px-1.5 py-0.5 rounded text-[10px] font-bold text-violet-600 bg-violet-100/50 border border-violet-200 hover:bg-violet-200/50 dark:text-violet-300 dark:bg-violet-900/30 dark:border-violet-800 transition-colors"
          >
            <span className="text-sm leading-none">+</span> 间视
          </button>
          <div className="w-px h-3 bg-slate-300 dark:bg-gray-600 mx-0.5" />
          <button
            onClick={() => useLevelingStore.getState().addStation(station.id)}
            className="flex min-h-11 min-w-11 items-center gap-0.5 px-1.5 py-0.5 rounded text-[10px] font-bold text-blue-600 bg-blue-100/50 border border-blue-200 hover:bg-blue-200/50 dark:text-cyan-400 dark:bg-blue-900/30 dark:border-blue-800 transition-colors"
          >
            <span className="text-sm leading-none">+</span> 测站
          </button>
      ...
      <AnimatePresence>
        {showResults && (
          <div className="overflow-hidden bg-slate-50/40 dark:bg-gray-900/40 px-1.5 pb-1.5">
            ...
            <div className="grid grid-cols-4 divide-x divide-slate-200/60 dark:divide-gray-700/60 rounded-md bg-white/60 dark:bg-gray-800/60 border border-slate-200/80 dark:border-gray-700/80 shadow-sm mt-1">
            ...
            {(grade === '3' || grade === '4') && (
              <div className="grid grid-cols-3 divide-x divide-slate-200/60 dark:divide-gray-700/60 rounded-md bg-slate-100/60 dark:bg-gray-800/40 border border-slate-200/80 dark:border-gray-700/80 shadow-sm mt-1">
              ...
              <div className="mt-1 flex flex-col gap-1">
                {(readings.intermediates || []).map((inter, i) => {
                  ...
                  if (!interResult) return null;
                   ...
                   return (
                     <div key={inter.id} className="grid grid-cols-4 divide-x divide-violet-200/60 dark:divide-violet-900/40 rounded-md bg-violet-50/40 dark:bg-violet-900/10 border border-violet-100/80 dark:border-violet-800/30 shadow-sm">
